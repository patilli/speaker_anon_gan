...
from dataset import SpeakerEmbeddingsDataset
from training.train_wgan import train_gan
from utils import setup_logger, setup_tensorboard
from WGAN_QC.init_wgan import create_wgan
...
def get_args():
    ...
    parser.add_argument(
        ...
        default=Path("data_path/reference_embeddings_as_list.pt"),
        type=Path,
    )
    parser.add_argument("--gpu", help="GPU to use for training", action="append")
    parser.add_argument("--id", default=None)
    parser.add_argument("--config", type=str, default="./configs/gan_anon.json")
...
def main(args):
    ...
    experiment = get_comet_logging() if args.use_comet_ml_logging else None
